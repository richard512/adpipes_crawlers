...
class Crawler extends base.Base
  loginURL : 'http://portal.advertise.com/index/sign-in'
  IdURL : 'http://portal.advertise.com/rep_ver/report/get-id'
  GetURL : 'http://portal.advertise.com/rep_ver/report/get'
  GetVersionURL : 'http://portal.advertise.com/'
  ...
  init: ->
    ...
    @request @GetVersionURL, (err, resp, body) =>
      ...
      @IdURL = @IdURL.replace /rep_ver/, version
      @GetURL = @GetURL.replace /rep_ver/, version
  ...
  process : (err, resp, body, cb) ->
      params =
        ...
        form :
          ...
          'report[pre_send_now_email_addresses]': @username
